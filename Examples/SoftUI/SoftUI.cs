...
namespace SoftUI
{
    ...
    public sealed class ModMenuService
    {
        ...
        GameObject CreateButton(string label, Button style, Transform parent)
        {
            ...
            Image image = obj.AddComponent<Image>(); Image sourceImage = style == null ? null : style.GetComponent<Image>(); if (sourceImage != null) { image.sprite = sourceImage.sprite; image.material = sourceImage.material; image.type = sourceImage.type; image.color = sourceImage.color; } if (image.color.a < .15f) image.color = new Color(.28f, .23f, .18f, 1f);
            Button button = obj.AddComponent<Button>(); if (style != null) { button.colors = style.colors; button.transition = style.transition; button.spriteState = style.spriteState; button.animationTriggers = style.animationTriggers; button.navigation = style.navigation; } ColorBlock buttonColors = button.colors; if (buttonColors.normalColor.a < .15f) { buttonColors.normalColor = new Color(.28f, .23f, .18f, 1f); buttonColors.highlightedColor = new Color(.40f, .33f, .25f, 1f); buttonColors.pressedColor = new Color(.18f, .14f, .10f, 1f); button.colors = buttonColors; } button.targetGraphic = image;
    ...
    public sealed class SoftTab
    {
        ...
        public Button AddButton(string label, Action clicked) { var obj = Row(label); LayoutElement size = obj.GetComponent<LayoutElement>(); size.minWidth = 190f; size.preferredWidth = 230f; size.flexibleWidth = 0f; var image = obj.AddComponent<Image>(); image.color = new Color(.28f, .23f, .18f, 1f); var button = obj.AddComponent<Button>(); button.targetGraphic = image; button.onClick.AddListener(() => Safe(clicked)); SoftWindow.AddLabel(obj.transform, label.ToUpperInvariant(), 14, new Color(.94f, .9f, .82f, 1f), 34f).alignment = TextAnchor.MiddleCenter; return button; }
