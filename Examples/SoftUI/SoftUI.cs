...
namespace SoftUI
{
    ...
    public sealed class ModMenuService
    {
        ...
        internal void InstallMainMenuButton(GameObject root)
        {
            ...
            RectTransform modsRect = modsButton.GetComponent<RectTransform>(); modsRect.anchorMin = new Vector2(0f, 1f); modsRect.anchorMax = new Vector2(0f, 1f); modsRect.pivot = new Vector2(0f, 1f); modsRect.anchoredPosition = new Vector2(24f, -24f); modsRect.sizeDelta = new Vector2(104f, 32f);
            LayoutElement modsLayout = modsButton.GetComponent<LayoutElement>(); if (modsLayout != null) { modsLayout.minWidth = 104f; modsLayout.preferredWidth = 104f; modsLayout.minHeight = 32f; modsLayout.preferredHeight = 32f; modsLayout.flexibleWidth = 0f; }
        ...
        GameObject CreateButton(string label, Button style, Transform parent)
        {
            ...
            LayoutElement layout = obj.AddComponent<LayoutElement>(); layout.minWidth = 104f; layout.preferredWidth = 104f; layout.minHeight = 32f; layout.preferredHeight = 32f;
            Image image = obj.AddComponent<Image>(); Image sourceImage = style == null ? null : style.GetComponent<Image>(); if (sourceImage != null) { image.sprite = sourceImage.sprite; image.material = sourceImage.material; image.type = sourceImage.type; image.color = sourceImage.color; } if (image.color.a < .15f) image.color = new Color(.08f, .08f, .08f, .58f);
            Button button = obj.AddComponent<Button>(); if (style != null) { button.colors = style.colors; button.transition = style.transition; button.spriteState = style.spriteState; button.animationTriggers = style.animationTriggers; button.navigation = style.navigation; } ColorBlock buttonColors = button.colors; if (buttonColors.normalColor.a < .15f) { buttonColors.normalColor = new Color(.08f, .08f, .08f, .58f); buttonColors.highlightedColor = new Color(.35f, .35f, .35f, .72f); buttonColors.pressedColor = new Color(.02f, .02f, .02f, .72f); button.colors = buttonColors; } button.targetGraphic = image;
            ...
            Text sourceText = style == null ? null : style.GetComponentInChildren<Text>(true); Text text = textObj.AddComponent<Text>(); text.font = sourceText == null || sourceText.font == null ? Resources.GetBuiltinResource<Font>("Arial.ttf") : sourceText.font; text.fontSize = sourceText == null || sourceText.fontSize < 8 ? 12 : Mathf.Min(sourceText.fontSize, 14); text.color = sourceText == null || sourceText.color.a < .15f ? new Color(.95f, .95f, .95f, 1f) : sourceText.color; text.alignment = TextAnchor.MiddleCenter; text.horizontalOverflow = HorizontalWrapMode.Overflow; text.verticalOverflow = VerticalWrapMode.Truncate; text.text = label;
            return obj;
        }
    ...
    public sealed class SoftWindow
    {
        ...
        internal SoftWindow(string id, string title, Transform canvas, IModLogger log)
        {
            ...
            content = Make("Content", root.transform); content.AddComponent<LayoutElement>().flexibleWidth = 1f; VerticalLayoutGroup contentLayout = content.AddComponent<VerticalLayoutGroup>(); contentLayout.padding = new RectOffset(20, 20, 12, 12); contentLayout.childForceExpandWidth = false; contentLayout.childForceExpandHeight = false;
    ...
    public sealed class SoftTab
    {
        ...
        public Button AddButton(string label, Action clicked) { var obj = Row(label); LayoutElement size = obj.GetComponent<LayoutElement>(); size.minWidth = 160f; size.preferredWidth = 190f; size.flexibleWidth = 0f; var image = obj.AddComponent<Image>(); image.color = new Color(.08f, .08f, .08f, .62f); var button = obj.AddComponent<Button>(); button.targetGraphic = image; button.onClick.AddListener(() => Safe(clicked)); SoftWindow.AddLabel(obj.transform, label.ToUpperInvariant(), 12, new Color(.95f, .95f, .95f, 1f), 30f).alignment = TextAnchor.MiddleCenter; return button; }
        public Toggle AddToggle(string id, string label, bool value, Action<bool> changed) { var obj = Row(id); obj.AddComponent<Image>().color = new Color(.08f, .08f, .08f, .28f); var toggle = obj.AddComponent<Toggle>(); toggle.onValueChanged.AddListener(v => Safe(() => changed(v))); GameObject box = new GameObject("Checkbox"); box.transform.SetParent(obj.transform, false); RectTransform boxRect = box.AddComponent<RectTransform>(); boxRect.sizeDelta = new Vector2(22f, 22f); Image boxImage = box.AddComponent<Image>(); boxImage.color = new Color(.75f, .75f, .75f, .85f); toggle.targetGraphic = boxImage; GameObject check = new GameObject("Checkmark"); check.transform.SetParent(box.transform, false); RectTransform checkRect = check.AddComponent<RectTransform>(); checkRect.anchorMin = Vector2.zero; checkRect.anchorMax = Vector2.one; checkRect.offsetMin = new Vector2(4f, 4f); checkRect.offsetMax = new Vector2(-4f, -4f); Image checkImage = check.AddComponent<Image>(); checkImage.color = new Color(.15f, .75f, .35f, 1f); toggle.graphic = checkImage; toggle.isOn = value; SoftWindow.AddLabel(obj.transform, label.ToUpperInvariant(), 13, new Color(.94f, .94f, .94f, 1f), 30f); return toggle; }
        public Slider AddSlider(string id, string label, float value, float min, float max, Action<float> changed) { var obj = Row(id); SoftWindow.AddLabel(obj.transform, label.ToUpperInvariant(), 12, new Color(.94f, .94f, .94f, 1f), 30f); var sliderObj = new GameObject("Slider"); sliderObj.transform.SetParent(obj.transform, false); RectTransform sliderRect = sliderObj.AddComponent<RectTransform>(); sliderObj.AddComponent<LayoutElement>().flexibleWidth = 1f; Image track = sliderObj.AddComponent<Image>(); track.color = new Color(.5f, .5f, .5f, .55f); var slider = sliderObj.AddComponent<Slider>(); slider.minValue = min; slider.maxValue = max; slider.value = value; GameObject handle = new GameObject("Handle"); handle.transform.SetParent(sliderObj.transform, false); RectTransform handleRect = handle.AddComponent<RectTransform>(); handleRect.sizeDelta = new Vector2(18f, 18f); Image handleImage = handle.AddComponent<Image>(); handleImage.color = new Color(.9f, .9f, .9f, 1f); slider.handleRect = handleRect; slider.targetGraphic = handleImage; slider.onValueChanged.AddListener(v => Safe(() => changed(v))); return slider; }
